...
def gamma_distance(x, obs, axes):
    dx = x - obs
    d = np.linalg.norm(dx)

    mu_val = mu(x, obs, axes)
...
# ===== MODULATION MATRIX =====
def modulatedDS(x, obs_list, axes_list, rho_list, ree):
    ...
    for k in range(kObst):
        g = gamma_distance(x, obs_list[k], axes_list[k])
    ...
    for k in range(kObst):
        ...
        lambda1 = 1 - weight / (g_eff ** (1 / rho_list[k]))
        lambda2 = 1 + weight / (g_eff ** (1 / rho_list[k]))
        lambda3 = lambda2
...
try:
    ...
    while True:
        ...
        # ===== MDS =====
        M_total = modulatedDS(x_curr, obs_list, axes_list, rho_list, ree)
